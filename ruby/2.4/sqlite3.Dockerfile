...
COPY ruby-$RUBY_VERSION.tar.gz /tmp/ruby.tar.gz
...
RUN set -ex \
  ...
  # && gem update --system \
  && rm -r /usr/src/ruby \
  && rm -rf /usr/local/lib/ruby/gems/2.2.0/cache /var/log/dpkg.log /var/log/apt/* /usr/share/doc /usr/share/man
